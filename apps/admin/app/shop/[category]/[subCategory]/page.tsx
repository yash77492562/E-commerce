...
import { ProductDeleteButton } from '../../../productDelete/productDelete';
...
export default function CategoryPage() {
  ...
  const fetchProducts = async (category: string, subCategory: string) => {
    try {
      setIsLoading(true);
      const url = `/api/shop/${category}/${subCategory}`;
      ...
      <div className="hidden md:block">
        {uniqueCategories.map((category) => {
          const categorySubcategories = categories
            .filter(cat => cat.category === category && cat.subCategory !== null)
            .reduce((unique, cat) => {
              if (!unique.some(u => u.subCategory === cat.subCategory)) {
                unique.push(cat);
              }
              return unique;
              ...
              {hasSubcategories && (
                <div className="pl-2 mt-1 space-y-1">
                  {categorySubcategories.map((subCat) => (
                    <div
                      key={`${category}-${subCat.subCategory}-${Math.random()}`}
                      className={cn(
  ...
  useEffect(() => {
    ...
    const subCategoryFromUrl = decodeURIComponent(params.subCategory as string);
  ...
  return (
    <div className="w-full bg-ibisWhite">
